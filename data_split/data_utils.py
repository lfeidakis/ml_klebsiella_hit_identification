...
class DataSplitter:
    ...
    ###############################################################
    # SPECIFIC DATA SPLIT FUNCTIONS - Diane

    def specific_train_val_test_split(self, split_idsList, dataset_investigated, driams_long_table):
        split_id = pd.read_csv(split_idsList)
        merged_df = driams_long_table.merge(split_id, on='sample_id', how='left')
        filtered_df = merged_df[merged_df['dataset'] == dataset_investigated]
        
        # Further filter to keep only rows where Set == 'train'
        train_X = filtered_df[filtered_df['Set'] == 'train']
        train_X = train_X.drop(columns=['Set'])

        # Further filter to keep only rows where Set == 'train'
        test_X = filtered_df[filtered_df['Set'] == 'test']
        test_X = test_X.drop(columns=['Set'])

        # Further filter to keep only rows where Set == 'train'
        val_X = filtered_df[filtered_df['Set'] == 'validation']
        val_X = val_X.drop(columns=['Set'])
        return train_X, val_X, test_X        
